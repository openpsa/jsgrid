...
(function ($) {
	...
	hideModal: function (selector, o) {
		...
		fh = (fh.indexOf('px') > -1) ? parseFloat(fh) : fh;

		var frmgr, frmdata, editOrView = 'View', frmdata = 'viewProp';
		if (o.form === 'edit') {
		    editOrView = 'Frm';
		    frmdata = 'formProp';
		}

		frmgr = '#' + $.jgrid.jqID(editOrView + 'Grid_' + o.gb.substr(6));
	...
	createModal: function (aIDs, content, p, insertSelector, posSelector, appendsel, css) {
		...
		p.zIndex = (parentZ) ? parseInt(parentZ, 10) + 2 : 950;
	    }
	    var rtlt = 0;
	...
	createEl: function (eltype, options, vl, autowidth, ajaxso) {
		...
		case "password" :
		case "button" :
		    var role = (eltype === 'button') ? 'button' : 'textbox';
		    elem = document.createElement("input");
		    elem.type = eltype;
		    elem.value = vl;
		    setAttributes(elem, options);
	...
	isEmpty: function (val) {
	    return (val.match(/^\s+$/) || val === "") ? true : false;
	},
	...
	checkValues: function (val, valref, customobject, nam) {
	    var edtrul = customobject, i, nm, dft, len, g = this, cm = g.p.colModel;
		...
		if (edtrul.required === true && $.jgrid.isEmpty(val)) {
		...
		if (
		    edtrul.number === true
		    && !(rqfield === false && $.jgrid.isEmpty(val))
		    && isNaN(val)
		) {
		...
		if (
		    edtrul.minValue !== undefined
		    && !isNaN(edtrul.minValue)
		    && parseFloat(val) < parseFloat(edtrul.minValue)
		) {
		    return [false, nm + ": " + $.jgrid.edit.msg.minValue + " " + edtrul.minValue, ""];
		}

		if (
		    edtrul.maxValue !== undefined
		    && !isNaN(edtrul.maxValue)
		    && parseFloat(val) > parseFloat(edtrul.maxValue)
		) {
		...
		if (!(rqfield === false && $.jgrid.isEmpty(val))) {
		    ...
		    if (edtrul.time === true && !$.jgrid.checkTime(val)) {
